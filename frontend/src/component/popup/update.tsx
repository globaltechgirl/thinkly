...
import { QuizQuestion, QuizWithQuestions } from "@/types/quiz";
...
const QuestionBox = memo(({ qIdx, questions, setQuestions }: { qIdx: number; questions: QuestionInput[]; setQuestions: Dispatch<SetStateAction<QuestionInput[]>> }) => {
...
const Update: FC<{ onClose: () => void; quizData: QuizWithQuestions; onSuccess?: () => void;  }> = ({ onClose, quizData, onSuccess }) => {
  ...
  const { quiz, questions: quizQuestions } = quizData;
    ...
    if (quiz) {
      setFormData({
        ...
        endTime: quiz.endTime
          ? new Date(quiz.endTime).toISOString().slice(0, 16)
          : "",
      });
    }

    if (quizQuestions?.length) {
      const mapped = quizQuestions.map((q: QuizQuestion, _index: number) => {
        return {
          question: q.question ?? "",
          option1: q.options?.[0] || "",
          option2: q.options?.[1] || "",
      ...
      console.warn("No quizQuestions found");
    }
  ...
  const mapQuestionsToPayload = (questions: QuestionInput[]) => {
    return questions.map((q, _i) => {
      ...
      const correctOptionIndex =
        q.correctOption != null ? q.correctOption : 0;
  ...
  const { updateQuiz, loadingRef } = useQuiz();
  const isUpdating = loadingRef.current.update;

  const handleUpdateQuiz = async (status: QuizStatus) => {
    try {
      ...
      <Box ref={scrollRef} onClick={(e) => e.stopPropagation()} style={styles.wrapper} className="scroll">
        <Box style={styles.main}>
          ...
          <Box style={{ ...styles.content, marginTop: -15 }}>
            {questions.map((_, qIdx) => ( <QuestionBox key={qIdx} qIdx={qIdx} questions={questions} setQuestions={setQuestions} /> ))}
          </Box>
          ...
          <Box style={styles.buttons}>
            ...
            <Box 
              style={{ ...styles.button2, backgroundColor: hoveredButton === "save" ? "var(--light-400)" : "var(--light-200)", opacity: isUpdating ? 0.6 : 1, pointerEvents: isUpdating ? "none" : "auto", }} onClick={() => handleUpdateQuiz("DRAFT")}
