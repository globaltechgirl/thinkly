...
const Bars: FC = () => {
  ...
  const { admin, fetchAdminAnalytics } = useAnalytics();
  useEffect(() => {
    fetchAdminAnalytics();
  }, [fetchAdminAnalytics]);

  const trend: TrendData[] = useMemo(() => {
    const apiData: MonthlySeries[] = admin?.monthlySubmissions ?? [];

    return MONTHS.map((_, index) => {
      const monthNumber = index + 1;
      const match = apiData.find(
        (item) => (item._id?.month ?? item.month) === monthNumber,
      );
      const year = match?.year ?? new Date().getFullYear();

      return {
        label: match?.monthName ?? `${MONTHS[index]} ${year}`,
        total: match?.total ?? 0,
      };
    });
  }, [admin]);
