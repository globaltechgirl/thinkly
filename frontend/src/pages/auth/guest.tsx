...
const Guest: FC = () => {
  ...
  const handleGuest = async () => {
    if (!formData.fullName.trim() || !formData.email.trim()) {
      notifyErrorOnce("Please fill in all fields");
      return;
    }
    ...
    try {
      ...
      notifySuccess("Guest session created");

      navigate(`${ROUTES.QUIZ}/${quiz.id}`, {
        replace: true,
        state: {
          quiz,
          guest: authService.getUser(),
        },
      });
    } catch (err) {
      notifyErrorOnce(
          ...
          <Box style={{ ...styles.body, width: isSmallScreen ? "85%" : "70%" }}>
            <Box style={styles.main}>
              ...
              <Text style={{ ...styles.subtitle, ...baseAnim(0.15) }}>
                Please enter your details to continue as a guest participant
              </Text>
            </Box>
            ...
            <Box style={styles.inputs}>
              ...
              <Box
                ...
                onClick={!loading ? handleGuest : undefined}
                onMouseEnter={(e) => {
                  if (!loading) {
              ...
              >
                {loading ? "Authenticating..." : "Join Quiz"}
              </Box>
            </Box>
          </Box>

          <Box style={{ ...styles.bottom, ...baseAnim(0.58) }}>
